...
Market artık joker alıp sattığı ve `JokerInventory.MaxSlots = 5` slot sınırı geldiği için
Parazit'in somut bir anlamı oldu. Verdiğim kararlar (hiçbiri tasarım dokümanında yoktu,
hepsi tek satırla değişir):

1. **Bağlı joker slot işgal etmez** (`JokerInventory.OccupiedSlots` onu saymaz). Jokerin
   tüm kazancı bu. Parazit'in kendisi slotunu tutmaya devam eder.
2. **Aynı anda tek bağlantı.** Yenisi için mevcut konağın ölmesi ya da Parazit'in gitmesi
   gerekir.
3. **Bağlı joker bloğu nerede olursa olsun çalışır** (deste/el/tahta). "Sadece küp tahtadayken"
   deseydik joker zamanın çoğunu uykuda geçirirdi.
4. **Parazit envanterden çıkarsa** bağlı joker silinmez, normal slota geri döner.

Konak küp takibi: tahtadaki küpler sadece kaynak KARTINI taşıyor, bloğun kaçıncı küpü
olduğunu değil. Bu yüzden bağlantı bir hücre indeksi tutuyor ve kart oynandığı anda
`TurnReport.PlacedCells[index]` ile küpün gerçekte nereye düştüğü öğreniliyor — bu,
döndürülmüş (Mechanical) ve yeniden şekillendirilmiş (Fox) bloklarda da doğru çalışır.

Giriş noktası: `GameSession.TryAttachJokerToCard(jokerInstanceId, cardId, cellIndex)`,
market fazında çağrılır. **Arayüzü henüz yok** — market ekranında üç seçim (joker, kart,
küp) sorabilen bir panel gerekiyor, o da enes'in `MarketView` dosyasında.

**Kalan tek joker: Powerbank.** Güçler sistemi hâlâ yok.

## 0.a Önceki durum — 35 jokerin 33'ü
...
- **İmitasyon** — el boyutu ıskartadaki kart sayısına eşit (en az 1). Tur sonunda elde
  kalanlar ıskartaya gider, sonra el yeni boyuta doldurulur. **`MaxHandSize` (varsayılan 8)
  koydum**, yoksa el çekme destesinden büyük olup oyuncunun hatası olmadan raunt kaybettirir.
- **Fraksiyon** — raunt başında ve çekme destesi bitince desteler karılıp ikiye bölünür,
  ıskartanın yarısı görünür olur; sonraki bölünmeye kadar oyuncu desteleri bir kez takas
  edebilir.

Yeni deste primitifleri: `RoundDeck.InsertRandomIntoDraw`, `SwapPiles`, `MergeAndSplitHalves`;
`RoundEngine.DiscardWholeHand` / `RefillHandToSize` (İmitasyon'un sıralaması için ayrıldı);
`RoundRules.RevealedDiscardCount` (Fraksiyon, sadece arayüz için).

## 0.b Ana liste — 31 jokerin 29'u yazıldı

**Yazılmayan 2 joker ve sebebi:**

- **Powerbank** — bir *gücün* kullanımını doldurur. Güçler sistemi henüz yok, dolayısıyla
  dolduracak bir şey yok. `Joker.OnPowerUsed` kancası hazır bekliyor.
- **Parazit** — bir jokeri destedeki bir bloğun küpüne bağlar. Çalışma zamanı yarısı hazır
  (`Joker.Attachment` alanı, ve yok edilen küpler kaynak kartıyla raporlanıyor), eksik olan
  market fazında "şu jokeri şu bloğa tak" diyebileceğin arayüz eylemi ve `BlockCard`
  üzerinde bir takı yuvası.

Geri kalan 29 joker yazıldı, kayıtlı (`JokerRegistry`) ve test edildi. Sayılar
(streak eşikleri, bonus miktarları, üst sınırlar) placeholder — hepsi joker sınıflarında
public alan, serbestçe ayarlanabilir.

### Bu turda eklenen motor primitifleri

`GameBoard`: `SetCubeKind` (küp türü değiştirme), `DestroyCubeForced` (obsidyeni kırmak
için), `CellsOfKind`, `Neighbours`, `IsOnEdge`, `SnapshotInto`.
`RoundEngine`: yok edilen küplerin türü ve kaynak kartıyla loglanması (`TurnReport.
DestroyedCubes`), "blok tek seferde tümüyle patladı" tespiti (`CardsFullyDestroyed`),
`DeclareLoss`, `ForceCleanSweep`, `SuppressNaturalSweep`, `TakeCardFromPiles`,
`CubesDestroyedThisTurn`.
Yeni küp türleri: `Ice` (Buzluk), `Void` (Kara delik tuzağı — üstüne konan küpü yutar).
Yeni kayıp nedeni: `LossReason.BetFailed` (Batak).

### Uygulanırken verilen kararlar (hepsi değiştirilebilir)
...
- **Buzluk**: duvara değen su tur sonunda donar; buz temizliği engellemez ama patlar ve
  ekstra puan verir.
- **Kara delik**: boşluk bloğu ıskartaya girer, aynı anda en fazla 2 tane yaşar, üstüne
  konan küpü yutar ve kendisi de yok olur. Kartlar raunt-içi (oyun destesine girmez).
- **Batak** ödül eğrisi: `MaxMultiplier * (1 - (N-1)/(ZeroAtTurns-1))`, erken bitirirsen
  oransal (7 dedin 3'te bitirdin → 7-turluk ödülün 3/7'si). Kaçırırsan raunt biter.
- **Seri tetik** eşik geçilince kapanır; eşiği geçiren tur el değiştirmez.

**Doğrulama:** 149 assert geçiyor (60 fuzzlanmış koşu, artık 29 jokerin rastgele
kombinasyonlarıyla), enes'in oyun izi 31 jokerle de birebir aynı, View+Core Unity
assembly'lerine karşı temiz derleniyor.

## 0.a Önceki durum notu — ilk dalga
...
REVISED 2026-07-25: the base sell value is no longer a per-joker number on the base type. It is DERIVED from the buy price — `MarketConfig.JokerSellValue(rarity) = round(JokerBuyPrice(rarity) * ContentSellFraction)`, one global 60% rate shared by jokers and powers — and `JokerInventory.SellValueOf(joker)` adds the earned parts on top: base + AccruedValue + AuctionPremium. The hand-written per-item BaseSellValue (15-70) it replaced had drifted ABOVE the flat 40 buy price on most commons, so buying a joker and selling it back printed money; deriving sell from buy makes sell < buy true by construction at every rarity. Earned value is still paid at FULL value — only the base is cut. The original design below still describes AccruedValue and AuctionPremium correctly.
