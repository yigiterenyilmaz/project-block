...
//
//   * The compressed cube stands on the cell the PLAYER chose inside the patch (PressCell) - the
//     anchor, its bottom-left cell, unless they picked another corner - never its centre.
//   * There is no single expansion direction. The press cell is restored in place; the cell beside
//     it is pushed straight away along x, the one above/below it along y, and only the DIAGONAL
//     has a choice: horizontal first, vertical if that is shut. That diagonal is the one place in the power
...
namespace ProjectBlock.Core
{
    ...
    {
        internal PressCompressionVisuals(GridPos anchor, GridPos pressCell)
        {
            Anchor = anchor;
            PressCell = pressCell;
        }

        /// <summary>The patch's bottom-left cell - the name of the patch.</summary>
        public GridPos Anchor { get; private set; }

        /// <summary>The cell of the patch the player chose to keep the compressed cube. Equal to
        /// Anchor unless they picked another corner; never a centre.</summary>
        public GridPos PressCell { get; private set; }
        ...
        /// <summary>Where the compressed cube ended up - the PressCell the player chose. This is
        /// what the animation converges on.</summary>
    ...
    {
        internal PressReleaseVisuals(GridPos anchor, GridPos pressCell)
        {
            Anchor = anchor;
            PressCell = pressCell;
        }

        /// <summary>The patch's bottom-left cell. A failure's blast is centred on the patch, so
        /// this is also the centre of a detonation.</summary>
        public GridPos Anchor { get; private set; }

        /// <summary>The cell the compressed cube stood in and opened from - Anchor unless the
        /// player chose another corner.</summary>
        public GridPos PressCell { get; private set; }

        private readonly List<GridPos> cells = new List<GridPos>();

        /// <summary>The four cells it wants back, in patch order (anchor, right, up, up-right). The
        /// one equal to PressCell is never pushed for - it is already the press's own cell.</summary>
        public IReadOnlyList<GridPos> Cells
        {
            get { return cells; }
        }
